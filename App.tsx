...
import { Download, Upload, Plus, RefreshCw, Grid as GridIcon, Square, ArrowRight, ArrowDown, Trash2, ChevronRight, ChevronDown, ChevronsUpDown, Settings, Eye, EyeOff, X, Wand2 } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const createPaletteItem = () => {
    ...
    if (existingIndex !== -1) {
      ...
      // Update timestamp to now, keep ID or generate new? 
      // Keeping ID is safer for list keys if we were using it for that, 
      // but essentially it's the same item moved.
  ...
  const applyPreset = (definition: PresetDefinition) => {
    if (definition.type === 'solid') {
      ...
      if (selection && gridRef.current) {
         // Create a temporary item for applying (don't save to history automatically unless user clicks +)
        ...
        <aside className="w-72 bg-gray-900 border-l border-gray-800 flex flex-col shrink-0 z-20 shadow-xl">
          <div className="p-4 flex-1 overflow-y-auto custom-scrollbar flex flex-col">
            ...
            <div className="mb-5 space-y-2 shrink-0">
              <h3 className="text-[10px] font-bold uppercase tracking-wider text-gray-500">Fill Mode</h3>
              
              <div className="grid grid-cols-3 gap-1">
                <button
                  onClick={() => setMode('solid')}
                  className={`flex flex-col items-center justify-center p-2 rounded border transition-all ${mode === 'solid' ? 'bg-blue-600/20 border-blue-500 text-blue-400' : 'bg-gray-800 border-gray-700 text-gray-400 hover:bg-gray-750'}`}
                >
                ...
                <button
                  ...
                  className={`flex flex-col items-center justify-center p-2 rounded border transition-all ${mode === 'linear-horizontal' ? 'bg-blue-600/20 border-blue-500 text-blue-400' : 'bg-gray-800 border-gray-700 text-gray-400 hover:bg-gray-750'}`}
                >
                  <ArrowRight size={16} className="mb-1" />
                  <span className="text-[10px]">H-Grad</span>
                </button>
                <button
                  onClick={() => setMode('linear-vertical')}
                  className={`flex flex-col items-center justify-center p-2 rounded border transition-all ${mode === 'linear-vertical' ? 'bg-blue-600/20 border-blue-500 text-blue-400' : 'bg-gray-800 border-gray-700 text-gray-400 hover:bg-gray-750'}`}
                >
                  <ArrowDown size={16} className="mb-1" />
                  <span className="text-[10px]">V-Grad</span>
                </button>
              </div>
            </div>
